...
describe("assignment2", () => {
  ...
  it("login_add_two_items_to_cart_checkout_enter_details_then_continue_and_verify", function () {
    ...
    cy.enterDetails(
      ...
      this.data.lastName,
      this.data.postalCode
    ); //enter the details at checkout page
    cy.clickContinue(); //click on continue button
    // all verifications
    cy.verifyCheckout(
      "#item_1_title_link > .inventory_item_name",
      this.text.boltTshirt
    ); //first item name
    cy.verifyCheckout(
  ...
  it("login_add_item_goto_cart_checkout_leave_details_empty_and verify_error_message", function () {
    ...
    cy.emptyVerify(
      ...
      this.data.lastName,
      this.data.postalCode,
      this.text.errorFirstName
    ); //keeping First Name text field empty, fill details
    ...
    cy.emptyVerify(
      ...
      "{alt}",
      this.data.postalCode,
      this.text.errorLastName
    ); //keeping Last Name text field empty, fill details
    ...
    cy.emptyVerify(
      ...
      this.data.lastName,
      "{alt}",
      this.text.errorPostalCode1
    ); //keeping Postal Code text field empty, fill details
  });

  // fourth exercise
  it("login_sort_highToLow_and_verify_highest_price_item_should_be_in_the_first_place", function () {
    cy.login(this.data.user1, this.data.password1); //login to the application
    ...
    cy.get(this.locator.highestPriceItem).should(
      "have.text",
      this.text.highestItemPrice
    );
  });
  it("Fill the Online Form", () => {
    cy.intercept('http://xyz.com/model').as('getModel') //Intercept the url which is triggered after clicking the form model
    cy.get("#form-model").click();
    cy.wait('@getModel') // Wait till model is not open
    cy.get("#form-model").should("exist");
    cy.get("#form-name").should("be.visible");
    cy.contains("FirstName").should('be.visible')
    cy.get("#first-name").type("Harry");
    cy.contains("LastName").should('be.visible')
    cy.get("#last-name").type("Smith");
    cy.contains("Email").should('be.visible')
    cy.get("#email").type("Harry@yopmail.com");
